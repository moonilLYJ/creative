...
# 페이지 설정
st.set_page_config(
    page_title="유소년 축구 맞춤형 운동 추천 앱",
    page_icon="⚽",
    layout="wide"
)

# 유소년 축구 맞춤형 운동 데이터베이스 (총 8가지 단점)
exercise_database = {
    "순발력": {
        "맨몸 운동": [
...
# 화면 구성
st.title("⚽ 유소년 축구 선수 맞춤형 운동 추천 앱")
st.markdown("선수의 **단점**을 선택하면 그에 맞는 **맨몸, 기구, 웨이트 운동**을 한눈에 확인할 수 있습니다.")
st.markdown("---")

# 사이드바 메뉴
st.sidebar.header("🎯 진단 메뉴")
selected_weakness = st.sidebar.selectbox(
    "개선하고 싶은 단점을 선택하세요:",
    options=list(exercise_database.keys())
)

# 선택된 결과 출력
st.subheader(f"📌 [{selected_weakness}] 개선 맞춤 트레이닝 루틴")
